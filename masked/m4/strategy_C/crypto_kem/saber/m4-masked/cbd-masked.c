...
void cbd_masked_single(uint16_t s0[SABER_N], uint16_t s1[SABER_N], const uint8_t coins0[SABER_POLYCOINBYTES], const uint8_t coins1[SABER_POLYCOINBYTES]){
    ...
    for (j = 0; j < SABER_N / 32; j++) { //creates 32 samples at a time
        ...
        pack_bitslice(x_bit[0], y_bit[0], &coins0[32 * j], scratch0, scratch1);
        pack_bitslice(x_bit[1], y_bit[1], &coins1[32 * j], scratch1, scratch0);

        SecBitAddBitSubConsAdd(x_bit, y_bit, z_bit);

        unpack_bitslice(&s0[32 * j], z_bit[0]);
        unpack_bitslice(&s1[32 * j], z_bit[1]);

        for (k = 0; k < 32; k += 2) {
            B2A_Goubin(&s0[32 * j + k], &s1[32 * j + k]);
        }
    }
...
void cbd_masked(uint16_t s[SABER_SHARES][SABER_L][SABER_N], const uint8_t coins[SABER_SHARES][SABER_L * SABER_POLYCOINBYTES]){
    ...
    int32_t i;
    for(i = 0; i < SABER_L; i++){
        cbd_masked_single(s[0][i], s[1][i], &coins[0][i * SABER_POLYCOINBYTES], &coins[1][i * SABER_POLYCOINBYTES]);
    }
