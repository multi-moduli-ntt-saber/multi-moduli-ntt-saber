...
int main(){
    ...
    int twiddle[((NTT_N - 1) / 7) << 3];
    gen_streamlined_CT_negacyclic_table(
        twiddle,
        RmodQ1, omegaQ1, Q1,
        &profile, 1);

    flag = 0;
    for(int i = 0; i < (((NTT_N - 1) / 7) << 3); i++){
        if(streamlined_CT_negacyclic_table_Q1[i] != twiddle[i]){
            flag = 1;
            printf("%d: %d, %d\n", i, streamlined_CT_negacyclic_table_Q1[i], twiddle[i]);
